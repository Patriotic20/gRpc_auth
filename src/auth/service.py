...
class AuthService(auth_pb2_grpc.AuthServiceServicer):
    def __init__(self, session: AsyncSession):
        ...
        logger.info("AuthService initialized with DB session")
    ...
    async def Login(self, request, context):
        ...
        user_credentials = UserRequest(
            username=request.username,
            password=request.password,
        )

        user_data = await authenticate_user(session=self.session, user_credential=user_credentials)
    ...
    async def ValidateToken(self, request, context):
        ...
        async with db_helper.session_factory() as session:
            ...
            logger.info(f"Token validated for user: {username}, role: {roles[0].name if roles else None}")
